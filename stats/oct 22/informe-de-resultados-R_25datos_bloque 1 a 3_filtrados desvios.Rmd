...
medias_alt <- datos %>% 
  filter(ALT > 0) %>% 
  group_by(Bloque,variedad,Manejo, parcela, Línea)  %>% 
...
datos_alt <- datos %>% 
#  filter(ALT > medias_alt$media_ALT-1*medias_alt$desvio_ALT &
#           ALT < medias_alt$media_ALT+2*medias_alt$desvio_ALT)
  filter(ALT > 1)

medias_alt <- datos_alt %>% 
  filter(ALT > 0) %>% 
...
datos_dap <- datos %>% 
  filter(DAP > medias_dap$media_DAP-1*medias_dap$desvio_DAP &
           DAP < medias_dap$media_DAP+2*medias_dap$desvio_DAP)
...
library(nlme)
mDAP_nl <- lme(mediana ~ variedad*Manejo , random = ~ 1 | (Bloque/parcela), medias_dap)
anova(mDAP_nl)
```
...
```{r, echo=F}
# anova
mDAP_mna <- aov(mediana ~ Bloque+ variedad*Manejo + Error(parcela), medias_dap)
summary(mDAP_mna)

library(nlme)
mDAP_nl_mna <- lme(mediana ~ variedad*Manejo , random = ~ 1 | (Bloque/parcela), medias_dap)
anova(mDAP_nl_mna)
```
...
lsm_dap_int <- emmeans(mDAP, ~ variedad|Manejo)
...
```{r, echo=F}
# anova
mALT <- aov(media_ALT ~ Bloque+ variedad*Manejo + Error(parcela), medias_alt)
summary(mALT)

library(nlme)
mALT_nl <- lme(mediana ~ variedad*Manejo , random = ~ 1 | (Bloque/parcela), medias_alt)
anova(mALT_nl)
```
