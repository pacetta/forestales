...
```{r, echo=F}
datos <- import("Datos y Analisis Infostat La Lehmann San Agustín Joni 24-8-22.xlsx") %>% 
...
library(nlme)
mDAP_nl <- lme(mediana ~ variedad*tratamiento , random = ~ 1 | (bloque/parcela), medias_dap)
anova(mDAP_nl)
```
...
```{r, echo=F}
# anova
mDAP_mna <- aov(mediana ~ bloque+ variedad*tratamiento + Error(parcela), medias_dap_2)
summary(mDAP_mna)

library(nlme)
mDAP_nl_mna <- lme(mediana ~ variedad*tratamiento , random = ~ 1 | (bloque/parcela), medias_dap_2)
anova(mDAP_nl_mna)
```
...
#  filter(tratamiento == "T") %>% 
  ...
  geom_point() +
  geom_line(aes(group = tratamiento), stat = "summary", fun.y = mean)+
  labs(x = "Variedad", color = "Tratamiento", y =  "DAP (cm)") 
```
