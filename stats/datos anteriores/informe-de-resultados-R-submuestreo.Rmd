...
```{r, echo=F}
# grafico de medias 
medias2 <- mediana_datos  %>% 
  mutate(parcela  = as.factor(paste(bloque , variedad , sep= "-")))

medias2 %>% 
  ggplot(.)+
  aes(x = variedad, y = mediana_DAP, color = tratamiento) +
  geom_point() +
...
medias2 %>% 
  ggplot(.)+
  aes(x = tratamiento, y = mediana_DAP, color = variedad) +
  geom_point() +
...
writexl::write_xlsx(medias_bloque3, "mediana_datos_bloque3.xlsx")
...
# modelo DAP
# supuestos
mtDAP <- aov(mediana_DAP ~ variedad*tratamiento, medias2)
...
```{r, echo=F}
# anova
mDAP <- aov(mediana_DAP ~ bloque + variedad*tratamiento  + 
              Error(parcela), medias2)
summary(mDAP)
```
...
#colSums(K)
#crossprod(K[ , 1], K[ , 2])
contrast(lsm_dap_v, method = list(K))
```
...
medias2 %>% 
  ggplot(.)+
  aes(x = variedad, y = mediana_ALT, color = tratamiento) +
  geom_point() +
  geom_line(aes(group = tratamiento), stat = "summary", fun.y = mean)+
...
# modelo ALT
# supuestos
mtALT <- aov(mediana_ALT ~ variedad*tratamiento, medias2)
...
```{r, echo=F}
# anova
mALT <- aov(mediana_ALT ~ bloque+ variedad*tratamiento + Error(parcela), medias2)
summary(mALT)
```
